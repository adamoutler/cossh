...
- **Communication & Artifacts:** All discussion and communication regarding a task MUST happen via comments on the Kanban ticket. Do not communicate via markdown files. The `docs/qa/` directory is strictly reserved for hard artifacts (logs, screenshots, receipts). 
- **Rigorous Proof & Closure:** You must gather irrefutable proof of completion as artifacts in the `docs/qa/` directory (e.g., `docs/qa/SSH-1.md`). Transition tickets using the `mcp_kanban_complete_work` tool and explicitly record your comments and reference the proof artifacts on the ticket. The `@reality-checker` performs a rigorous audit; vague assertions will be rejected. **If the reality-checker rejects the transition, you MUST halt. Do not proceed to other tasks. You must read the feedback on the ticket, address the deficiencies, gather new proof, and repeat the QA loop until approved.**
