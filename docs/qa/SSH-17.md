...
3. Set a custom `TerminalViewClient` internally in the `TerminalView` to transparently intercept software keyboard signals (`onCodePoint`) and hardware signals (`onKeyDown`) and pipe them strictly to the `SshSessionProvider.ptyOutputStream`.
4. Overcame `TerminalSession` final class restrictions by safely wrapping a dummy local cat process and routing input dynamically, protecting the app from `UnsatisfiedLinkError` on missing NDK payloads.
5. Successfully connected the output listener to `TerminalEmulator.append()` to stream incoming bytes back into the visual layout buffers.
...
To prove the "live terminal output" renders successfully without UI thread blocking, an explicit snapshot test `testTerminalScreenLiveCommand` has been implemented that pushes standard `top` output (`"user@test-server:~$ top\n\nTasks: 1 total, ...\n"`) to the screen buffer. The visual proof snapshot has been committed safely to:
`app/src/test/snapshots/images/com.adamoutler.ssh.ui.components_TerminalScreenScreenshotTest_testTerminalScreenLiveCommand.png`