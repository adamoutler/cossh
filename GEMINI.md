...
- **Facilitate Verification:** Make the reality checker's job faster and easier by explicitly pointing out the exact locations of your proof in your comments (e.g., `docs/qa/my-file.png` or `docs/qa/SSH-22.md`). The reality checker evaluates based on the evidence provided in the ticket, so direct citations of committed artifacts are essential. **Never claim the ticket is "ready" or "complete" in the comment text itself; the act of transitioning the ticket implicitly makes that claim. Your comment should strictly provide the proof and context required to validate that claim.**
...
- **Test Output Requirements:** The CI pipeline runs `./gradlew testDebugUnitTest jacocoTestReport`. To ensure reports are generated:
  ...
  - Integration tests using `mock_sshd.py` (like `FiftyKbIntegrityTest`) must have long timeouts (`withTimeout(120_000L)`) and use `CountDownLatch` to wait for the Python server to start and listen (e.g. `latch.await(45, TimeUnit.SECONDS)`) to avoid arbitrary connection timeouts on slow GitHub Action runners. If they are too flaky, use `@org.junit.Ignore` to prevent them from failing the build and masking coverage.
  - The Gradle test task timeout MUST be set to `Duration.ofMinutes(30)` or higher in `build.gradle.kts` (e.g., `tasks.withType<Test> { timeout.set(Duration.ofMinutes(30)) }`) because the GitHub Actions runners are slow and the test suite takes >11 minutes to complete.
